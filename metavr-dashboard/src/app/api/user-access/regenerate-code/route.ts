...
export async function POST(request: NextRequest) {
  try {
    ...
    const backendEndpoint = getBackendEndpoint(request, '/user-access/regenerate-code');
    ...
    if (!contentType || !contentType.includes('application/json')) {
      ...
      console.error('[UserAccessRegenerate] Backend returned non-JSON response:', text.substring(0, 200));
      return NextResponse.json(
    ...
    if (!response.ok) {
      return NextResponse.json(
        { success: false, message: data.message || 'Failed to regenerate access code' },
