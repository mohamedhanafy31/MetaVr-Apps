...
export async function POST(request: NextRequest) {
  try {
    ...
    const backendEndpoint = getBackendEndpoint(request, '/user-access/toggle-access');
    ...
    if (!contentType || !contentType.includes('application/json')) {
      ...
      console.error('[UserAccessToggle] Backend returned non-JSON response:', text.substring(0, 200));
      return NextResponse.json(
    ...
    if (!response.ok) {
      return NextResponse.json(
        { success: false, message: data.message || 'Failed to toggle access' },
    ...
    return NextResponse.json(
      { success: false, message: 'Failed to toggle access' },
      { status: 500 }
    );
  }
}
