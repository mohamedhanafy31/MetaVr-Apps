...
export async function POST(request: NextRequest) {
  try {
    ...
    const backendEndpoint = getBackendEndpoint(request, '/user-access/approve');
    ...
    if (!contentType || !contentType.includes('application/json')) {
      ...
      console.error('[UserAccessApprove] Backend returned non-JSON response:', text.substring(0, 200));
      return NextResponse.json(
    ...
    if (!response.ok) {
      return NextResponse.json(
        { success: false, message: data.message || 'Failed to approve access request' },
    ...
    return NextResponse.json(
      { success: false, message: 'Failed to approve access request' },
      { status: 500 }
    );
  }
}
