...
async function postClientLog(body: Record<string, unknown>) {
  try {
    ...
    console.warn('[card-matching] Failed to emit client log', error);
  }
}

export function useAppLogger(scope = 'CardMatchingApp') {
  ...
  const emit = useCallback(
    (level: LogLevel, message: string, options: LogOptions = {}) => {
      ...
      const payload = {
        ...
        data: {
          appKey: APP_KEY,
          appName: accessContext.appName || 'Card Matching',
          ...data,
        },
        meta: {
